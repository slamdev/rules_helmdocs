...
def _helmdocs_toolchain_impl(ctx):
    ...
    # Make the $(HELMDOCS_BIN) variable available in places like genrules.
    # See https://docs.bazel.build/versions/main/be/make-variables.html#custom_variables
    template_variables = platform_common.TemplateVariableInfo({
        "HELMDOCS_BIN": target_tool_path,
        # Variable to use with disabled legacy_external_runfiles
        "HELMDOCS_RUNFILES_BIN": target_tool_path.replace("external/", "../", 1),
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    helmdocsinfo = HelmdocsInfo(
...
helmdocs_toolchain = rule(
    implementation = _helmdocs_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a helmdocs compiler/runtime toolchain.
